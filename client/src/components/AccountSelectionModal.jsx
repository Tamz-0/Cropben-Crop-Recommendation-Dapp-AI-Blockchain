...
const AccountSelectionModal = ({ accounts, onSelect, onClose }) => {
  const shortAccount = (acc) => `${acc.substring(0, 6)}...${acc.substring(acc.length - 4)}`;
  ...
  return (
    ...
    >
      ...
      >
        <h2 className="text-xl font-bold mb-4 text-gray-800">Select an Account</h2>
        <div className="flex flex-col space-y-2">
          {accounts.map((acc) => (
            ...
            >
              <span className="font-mono text-gray-700">{shortAccount(acc)}</span>
            </button>
          ))}
        </div>
        <button 
