...
function RegistrationModal({ role, onClose, onSubmit, currentNickname }) {
    ...
    // Pre-fill the nickname if it's already set
    useEffect(() => {
        if (currentNickname) {
    ...
    const handleSubmit = (e) => {
        ...
        if (role === 'Farmer') {
            ...
            if (!formData.toolsOwned || isNaN(formData.toolsOwned) || Number(formData.toolsOwned) < 0) {
                setError('Please enter a valid number of tools owned.');
                return;
            }
        }
        onSubmit(formData); // Pass the validated data up to the App component
    };

    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50">
            <div className="bg-white rounded-lg shadow-2xl p-8 w-full max-w-md">
                <h2 className="text-2xl font-bold mb-4 text-gray-800">Register as a {role}</h2>
                <form onSubmit={handleSubmit}>
                    {/* --- Nickname Input --- */}
                    <div className="mb-4">
                        <label htmlFor="nickname" className="block text-gray-700 font-semibold mb-2">Nickname</label>
                        <input
                            type="text"
                            id="nickname"
                            name="nickname"
                    ...
                    {/* --- Conditional Farmer Inputs --- */}
                    {role === 'Farmer' && (
                        <>
                            <div className="mb-4">
                                <label htmlFor="landSize" className="block text-gray-700 font-semibold mb-2">Land Size (in acres)</label>
                                <input
                                    type="number"
                                    id="landSize"
                                    name="landSize"
                            ...
                            <div className="mb-6">
                                <label htmlFor="toolsOwned" className="block text-gray-700 font-semibold mb-2">Number of Tools Owned</label>
                                <input
                                    type="number"
                                    id="toolsOwned"
                                    name="toolsOwned"
