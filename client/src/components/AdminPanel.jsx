...
const AdminPanel = ({ web3, userRegistry, account, showNotification, triggerRefresh }) => {
    ...
    const addAdminRole = async (role) => {
        ...
        try {
            ...
            if (role === "Bank") {
                await userRegistry.methods.addBank(address, name).send({ from: account });
            } else if (role === "Insurance") {
                await userRegistry.methods.addInsurance(address, name).send({ from: account });
            } else if (role === "Verifier") {
                await userRegistry.methods.addVerifier(address, name).send({ from: account });
            }
    ...
    return (
        <div className="mt-8 p-6 bg-yellow-100 border-2 border-yellow-400 rounded shadow text-center">
            ...
            <p className="mt-2 text-gray-700">You are the contract owner. Use these functions to add official entities.</p>
            <div className="mt-4 flex justify-center flex-wrap gap-4">
                <button onClick={() => addAdminRole("Bank")} className="px-4 py-2 rounded bg-red-600 text-white">Add Bank</button>
                <button onClick={() => addAdminRole("Insurance")} className="px-4 py-2 rounded bg-indigo-600 text-white">Add Insurance Co.</button>
                <button onClick={() => addAdminRole("Verifier")} className="px-4 py-2 rounded bg-gray-700 text-white">Add Verifier</button>
            </div>
        </div>
    );
};
