...
module.exports = async function (deployer,network, accounts) {
  // Deploy the UserRegistry contract, as it has no dependencies.
  await deployer.deploy(UserRegistry);
  const userRegistry = await UserRegistry.deployed();
  console.log("UserRegistry deployed at:", userRegistry.address);

  // Deploy the other contracts, passing the UserRegistry address to their constructors
  // links them together on the blockchain.
  ...
  // For testing, register a few default users
  // The first account is the owner/admin by default
  const owner = accounts[0];
  await userRegistry.addVerifier(accounts[0], "Admin Verifier", { from: owner });
